...
namespace vMenuClient.menus
{
    public class WeaponLoadouts
    {
        ...
        private readonly Menu SavedLoadoutsMenu = new("已保存的配置", "已保存武器配置列表");
        ...
        public void CreateMenu()
        {
            ...
            var saveLoadout = new MenuItem("保存配置", "将你当前的武器保存到新的配置槽位。");
            var savedLoadoutsMenuBtn = new MenuItem("管理配置", "管理已保存的武器配置。") { Label = "→→→" };
            var enableDefaultLoadouts = new MenuCheckboxItem("重生时恢复默认配置", "如果你设置了默认配置，那么每当你(重新)生成时，你的配置将自动装备。", WeaponLoadoutsSetLoadoutOnRespawn);
            ...
            void RefreshSavedWeaponsMenu()
            {
                ...
                {
                    var btn = new MenuItem(sw.Key.Replace("vmenu_string_saved_weapon_loadout_", ""), "点击以管理此配置。") { Label = "→→→" };
            ...
            var spawnLoadout = new MenuItem("装备配置", "生成此已保存的武器配置。这将移除你所有当前的武器并将其替换为此已保存的槽位。");
            var renameLoadout = new MenuItem("重命名配置", "重命名此已保存的配置。");
            var cloneLoadout = new MenuItem("克隆配置", "将此已保存的配置克隆到新槽位。");
            var setDefaultLoadout = new MenuItem("设为默认配置", "将此配置设置为你的默认配置，在每次(重新)生成时使用。这将覆盖杂项设置菜单中的'恢复武器'选项。你可以在主武器配置菜单中切换此选项。");
            var replaceLoadout = new MenuItem("~r~替换配置", "~r~这将用你当前库存中的武器替换此已保存的槽位。此操作无法撤销！");
            var deleteLoadout = new MenuItem("~r~删除配置", "~r~这将删除此已保存的配置。此操作无法撤销！");
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (SaveWeaponLoadout("vmenu_string_saved_weapon_loadout_" + name))
                            {
                                ...
                                Notify.Success($"你的武器已保存为~g~<C>{name}</C>~s~。");
                            }
                            else
                            {
                    ...
                    else if (item == renameLoadout || item == cloneLoadout) // rename or clone
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                Notify.Success($"你的武器配置已{(item == renameLoadout ? "重命名" : "克隆")}为~g~<C>{newName}</C>~s~。");
