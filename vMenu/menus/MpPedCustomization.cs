...
namespace vMenuClient.menus
{
    public class MpPedCustomization
    {
        ...
        public Menu savedCharactersMenu = new("vMenu", "管理已保存角色");
        public Menu savedCharactersCategoryMenu = new("分类", "运行时更新！");
        public Menu inheritanceMenu = new("vMenu", "角色继承选项");
        public Menu appearanceMenu = new("vMenu", "角色外观选项");
        public Menu faceShapeMenu = new("vMenu", "角色脸部形状选项");
        ...
        internal MenuItem editPedBtn = new("编辑已保存角色", "这允许你编辑已保存角色的所有内容。更改将在你按下保存按钮后保存到该角色的存档文件中");

        // Need to be editable from other functions
        private readonly MenuListItem setCategoryBtn = new("设置角色分类", new List<string> { }, 0, "设置该角色的分类。选择以保存");
        private readonly MenuListItem categoryBtn = new("角色分类", new List<string> { }, 0, "设置该角色的分类");
        ...
        private void MakeCreateCharacterMenu(bool male, bool editPed = false)
        {
            ...
            else
            {
                ...
                {
                    Log("形状混合值保存不正确，其值高于可能的最大值。重置为最大值");
                    _shapeMixValue = 1f;
                }

                if (_skinMixValue > 1f)
                {
                    Log("皮肤混合值保存不正确，其值高于可能的最大值。重置为最大值");
                    _skinMixValue = 1f;
                }
            }
        ...
        private async Task<bool> SavePed()
        {
            ...
            if (isEdidtingPed)
            {
                ...
                {
                    Notify.Error("无法保存您的角色。原因未知。 :(");
                    return false;
                }
            }
            else
            {
                var name = await GetUserInput(windowTitle: "输入保存名称", maxInputLength: 30);
                ...
                else
                {
                    ...
                    {
                        Notify.Success($"你的角色 (~g~<C>{name}</C>~s~) 已经被保存");
                        Log($"Saved Character {name}. Data: {json}");
                        return true;
                    }
                    else
                    {
                        Notify.Error($"保存失败, 好像是这个名字 (~y~<C>{name}</C>~s~) 已经被你个傻逼用了");
                        return false;
                    }
                }
            }
        ...
        private void CreateMenu()
        {
            ...
            var savedCharacters = new MenuItem("保存的字符", "生成、编辑或删除您现有的已保存多人游戏角色。")
            {
                Label = "→→→"
            };
