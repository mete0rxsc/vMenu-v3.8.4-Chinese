...
namespace vMenuClient.menus
{
    public class WeaponOptions
    {
        ...
        private void CreateMenu()
        {
            ...
            // Create the menu.
            menu = new Menu(Game.Player.Name, "洛圣都好市民家中常备");

            var getAllWeapons = new MenuItem("获得所有居家好物", "获得所有枪");
            var removeAllWeapons = new MenuItem("移除所有武器", " 删除库存中的所有武器");
            var unlimitedAmmo = new MenuCheckboxItem("无限子弹", "无限弹药供应", UnlimitedAmmo);
            var noReload = new MenuCheckboxItem("不需要换蛋", "永远不换蛋", NoReload);
            var setAmmo = new MenuItem("设置所有弹药数", "设置所有武器的弹药量");
            var refillMaxAmmo = new MenuItem("重新装填所有子弹", "给你所有的武器最大弹药");
            var spawnByName = new MenuItem("按名称分类的武器", "输入要生成的武器名称");
            ...
            var addonWeaponsBtn = new MenuItem("附加武器", "装备/删除此服务器上可用的附加武器");
            ...
            foreach (var addonWeapon in ValidAddonWeapons.AddonWeaponsList)
            {
                ...
                {
                    var addonWeaponMenu = new Menu("武器选项", addonWeapon.Name)
                    ...
                    var addonWeaponItem = new MenuItem(addonWeapon.Name, $"打开以下选项 ~y~{addonWeapon.Name}~s~.")
                    {
                        Label = "→→→",
                    ...
                    var getOrRemoveWeapon = new MenuItem("装备/移除武器", "在您的库存中添加或删除此武器.")
                    {
                        LeftIcon = MenuItem.Icon.GUN
                    };
                    ...
                    var fillAmmo = new MenuItem("重新装填弹药", "获取此武器的最大弹药")
                    {
                    ...
                    var weaponTints = new MenuListItem("Tints", tints, 0, "为你的武器选择一种色调");
                    ...
                    addonWeaponMenu.OnItemSelect += (sender, item, index) =>
                    {
                        ...
                        else if (item == fillAmmo)
                        {
                            ...
                            {
                                Notify.Error("在重新装填弹药之前，你需要先拿到武器！");
                            }
                        }
                    };
                    #endregion
            ...
            if (addonWeaponsMenu.Size == 0)
            {
                ...
                addonWeaponsBtn.Description = "此服务器上没有此选项，因为您没有使用它的权限，或者它的设置不正确";
            ...
            if (IsAllowed(Permission.WPParachute))
            {
                ...
                var parachuteBtn = new MenuItem("降落伞选项", "所有与降落伞相关的选项都可以在此处更改") { Label = "→→→" };
            ...
            var spacer = GetSpacerMenuItem("↓ 你的武器库！ ↓");
            menu.AddMenuItem(spacer);

            var handGuns = new Menu("武器", "手枪");
            var handGunsBtn = new MenuItem("手枪");

            var rifles = new Menu("武器", "步枪");
            var riflesBtn = new MenuItem("步枪");

            var shotguns = new Menu("武器", "短小精悍散弹枪");
            var shotgunsBtn = new MenuItem("短小精悍散弹枪");

            var smgs = new Menu("武器", "秒男专用冲锋枪");
            var smgsBtn = new MenuItem("秒男专用冲锋枪");

            var throwables = new Menu("武器", "投掷物");
            var throwablesBtn = new MenuItem("投掷物");

            var melee = new Menu("武器", "近战武器");
            var meleeBtn = new MenuItem("近战武器");

            var heavy = new Menu("武器", "重火力");
            var heavyBtn = new MenuItem("重火力");

            var snipers = new Menu("武器", "豺狼的日子");
            var snipersBtn = new MenuItem("豺狼的日子");
            ...
            foreach (var weapon in ValidWeapons.WeaponList)
            {
                ...
                if (!string.IsNullOrEmpty(weapon.Name) && IsAllowed(weapon.Perm))
                {
                    ...
                    var weaponItem = new MenuItem(weapon.Name, $"打开以下选项 ~y~{weapon.Name}~s~.")
                    {
                        Label = "→→→",
                        LeftIcon = MenuItem.Icon.GUN,
                        ItemData = stats
                    ...
                    if (!IsAllowed(Permission.WPSpawn))
                    {
                        ...
                        getOrRemoveWeapon.Description = "您没有使用此选项的权限";
            ...
            menu.OnCheckboxChange += (sender, item, index, _checked) =>
            {
                ...
                {
                    NoReload = _checked;
                    Subtitle.Custom($"无需换蛋现在是 {(_checked ? "enabled" : "disabled")}.");
                }
                else if (item == unlimitedAmmo)
                {
                    UnlimitedAmmo = _checked;
                    Subtitle.Custom($"无限子弹现在是 {(_checked ? "enabled" : "disabled")}.");
                }
            };
            #endregion
