...
#pragma warning restore CS8793 // The given expression always matches the provided pattern.
        ...
        private static void PostPermissionsSetup()
        {
            ...
            Menu = new Menu(Game.Player.Name, "vMenu主菜单 - 汉化By.Mete0r");
            PlayerSubmenu = new Menu(Game.Player.Name, "玩家相关选项");
            VehicleSubmenu = new Menu(Game.Player.Name, "车辆相关选项");
            WorldSubmenu = new Menu(Game.Player.Name, "世界设置");
        ...
        private static void CreateSubmenus()
        {
            ...
            if (IsAllowed(Permission.OPMenu))
            {
                ...
                var button = new MenuItem("在线玩家", "所有现在在服务器上的玩家")
                {
                    Label = "→→→"
                };
            ...
            if (IsAllowed(Permission.OPUnban) || IsAllowed(Permission.OPViewBannedPlayers))
            {
                ...
                var button = new MenuItem("封禁的玩家", "看一看哪个小可爱被封禁了")
                {
                    Label = "→→→"
                };
                AddMenu(Menu, menu, button);
            ...
            if (IsAllowed(Permission.POMenu))
            {
                ...
                var button = new MenuItem("玩家选项", "常见的玩家选项可以在这里访问")
                {
                    Label = "→→→"
                };
            ...
            var vehicleSubmenuBtn = new MenuItem("车辆相关选项", "打开此子菜单以查看与车辆相关的子类别") { Label = "→→→" };
            ...
            if (IsAllowed(Permission.VSMenu))
            {
                ...
                var button = new MenuItem("刷车器", "按名称生成车辆或从特定类别中选择一辆.")
                {
                    Label = "→→→"
                };
                AddMenu(VehicleSubmenu, menu, button);
            ...
            if (IsAllowed(Permission.PVMenu))
            {
                ...
                var button = new MenuItem("个人车辆", "将车辆设置为您的私人车辆，并在不在车内时控制该车辆的一些事情.")
                {
                    Label = "→→→"
                };
                AddMenu(VehicleSubmenu, menu, button);
            ...
            if (IsAllowed(Permission.PAMenu))
            {
                ...
                var button2 = new MenuItem("MP踏板定制", "创建、编辑、保存和加载多人游戏。~r~注意，您只能保存在此子菜单中创建的peds。vMenu无法检测在此子菜单之外创建的peds。仅仅是由于GTA的限制.")
                {
                    Label = "→→→"
                };
            ...
            var worldSubmenuBtn = new MenuItem("世界相关选项", "打开此子菜单，查看与世界相关的子类别.") { Label = "→→→" };
            ...
            if (IsAllowed(Permission.NoClip))
            {
                var toggleNoclip = new MenuItem("切换无碰撞的Noclip模式", "这个是开关，按一下开，再按一下关");
            ...
            var btn = new MenuItem("关于vMenu 汉化By.Mete0r", "关于vMenu 汉化By.Mete0r 的信息.")
            {
                Label = "→→→"
            };
